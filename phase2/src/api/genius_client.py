...
class GeniusClient:
            ...
            if search_result.success:
                ...
                if hits:
                    ...
                    for hit in hits:
                        ...
                        # Handle various apostrophe/quote characters: ', ', ', ', ', '
                        apostrophe_variants = ["'", "'", "'", "'", "'", "'"]
                        normalized_song_name = song_name.lower()
                        normalized_result_title = result_title
                        
                        # Replace all apostrophe variants with a standard one
                        for variant in apostrophe_variants:
                            normalized_song_name = normalized_song_name.replace(variant, "'")
                            normalized_result_title = normalized_result_title.replace(variant, "'")
                        
                        if (normalized_song_name in normalized_result_title or 
                            normalized_result_title in normalized_song_name or
                            song_name.lower() in result_title or 
                            result_title in song_name.lower()):
    ...
    def get_song_details(self, song_id: int) -> Optional[Dict]:
        """Get detailed song information from Genius - returns song data even if credits are missing"""
        try:
            ...
            # Return the full song data - let caller extract what they need
            # This ensures we don't lose the song just because credit extraction fails
            return song_data
    ...
    def get_credits(self, title: str, artist: str) -> Dict:
        ...
        try:
            ...
            # Find the best result (prefer original over translations/covers)
            best_hit = None
            for hit in hits:
                song_title = hit['result']['title'].lower()
                song_artist = hit['result']['primary_artist']['name'].lower()
                
                # Skip translations, covers, and remixes
                if any(skip_word in song_title for skip_word in ['translation', 'srpski', 'cover', 'remix', '8-bit', 'karaoke']):
                    continue
                
                # Skip if artist is clearly not the original (e.g., "Genius srpski prevodi")
                if any(skip_word in song_artist for skip_word in ['translation', 'srpski', 'genius', 'prevod']):
                    continue
            ...
            if not best_hit:
                best_hit = hits[0]
                logger.warning(f"Using first result despite potential issues: {best_hit['result']['title']}")
            
            song_id = best_hit['result']['id']
            
            # Get detailed song information with enhanced credit extraction
            song_details = self._get_enhanced_credits(song_id)
            if song_details:
    ...
    def _get_enhanced_credits(self, song_id: int) -> Dict:
        """Enhanced credit extraction that handles multiple Genius API credit formats."""
        try:
            url = f"{self.BASE_URL}/songs/{song_id}"
            params = {'access_token': self.access_token} if self.access_token else {}
            ...
            # Initialize credit collections
            writers = set()
            producers = set()
            engineers = set()
            other_credits = set()
            
            # Method 1: Check song_credits field (standard format)
            if 'song_credits' in song_data and song_data['song_credits']:
                credits = song_data['song_credits']
                for credit in credits:
                    name = credit.get('name', '').strip()
                    role = credit.get('role', '').strip().lower()
                    if name:
                        if 'writer' in role or 'composer' in role or 'lyricist' in role:
                            writers.add(name)
                        elif 'producer' in role:
                            producers.add(name)
                        elif 'engineer' in role or 'mixing' in role or 'mastering' in role:
                            engineers.add(name)
                        else:
                            other_credits.add(f"{name} ({role})")
            
            # Method 2: Check producer_artists field
            if 'producer_artists' in song_data and song_data['producer_artists']:
                for producer in song_data['producer_artists']:
                    name = producer.get('name', '').strip()
                    if name:
                        producers.add(name)
            
            # Method 3: Check writer_artists field
            if 'writer_artists' in song_data and song_data['writer_artists']:
                for writer in song_data['writer_artists']:
                    name = writer.get('name', '').strip()
                    if name:
                        writers.add(name)
            
            # Method 4: Check custom_performances field (additional credits)
            if 'custom_performances' in song_data and song_data['custom_performances']:
                for performance in song_data['custom_performances']:
                    label = performance.get('label', '').strip().lower()
                    artists = performance.get('artists', [])
                    
                    for artist in artists:
                        name = artist.get('name', '').strip()
                        if name:
                            if 'writer' in label or 'composer' in label or 'lyricist' in label:
                                writers.add(name)
                            elif 'producer' in label:
                                producers.add(name)
                            elif 'engineer' in label or 'mixing' in label or 'mastering' in label or 'recording' in label:
                                engineers.add(name)
                            elif 'vocal' in label:
                                other_credits.add(f"{name} (Vocal Producer)")
                            elif 'publisher' in label:
                                other_credits.add(f"{name} (Publisher)")
                            else:
                                other_credits.add(f"{name} ({label.title()})")
            
            # Combine all credits
            all_writers = list(writers) if writers else []
            all_producers = list(producers) if producers else []
            all_engineers = list(engineers) if engineers else []
            all_other = list(other_credits) if other_credits else []
            ...
            if all_writers:
                result['writers'] = ', '.join(sorted(all_writers))
            else:
                result['writers'] = 'Information not available'
            
            if all_producers:
                result['producers'] = ', '.join(sorted(all_producers))
            else:
                result['producers'] = 'Information not available'
            
            # Add additional credits if we found any
            if all_engineers or all_other:
                additional_credits = []
                if all_engineers:
                    additional_credits.append(f"Engineers: {', '.join(sorted(all_engineers))}")
                if all_other:
                    additional_credits.append(f"Other: {', '.join(sorted(all_other))}")
                result['additional_credits'] = '; '.join(additional_credits)
            
            return result
            
        except Exception as e:
            logger.error(f"Error getting enhanced credits for song ID {song_id}: {e}")
            return None
...
class GeniusService:
    ...
    def get_song_metadata(self, song_name: str, artist_name: str) -> Dict:
        ...
        # Add additional credits (engineers, vocal producers, etc.)
        if credits_data.get('additional_credits'):
            additional_credits_text = credits_data['additional_credits']
            # Parse additional credits - they come in format "Engineers: name1, name2; Other: name3 (role), name4 (role)"
            for section in additional_credits_text.split(';'):
                section = section.strip()
                if ':' in section:
                    role_type, names_text = section.split(':', 1)
                    role_type = role_type.strip()
                    names_text = names_text.strip()
                    
                    for name_with_role in names_text.split(','):
                        name_with_role = name_with_role.strip()
                        if '(' in name_with_role and ')' in name_with_role:
                            # Format: "Name (Role)"
                            name, role = name_with_role.rsplit('(', 1)
                            name = name.strip()
                            role = role.rstrip(')').strip()
                        else:
                            # Format: just "Name" - use the role type
                            name = name_with_role
                            role = role_type
                        
                        if name and name != 'Information not available':
                            credits.append({
                                'name': name,
                                'id': None,
                                'role': role,
        ...
        if search_result.success:
            hits = search_result.data.get('response', {}).get('hits', [])
            if hits:
                # Use the same improved search logic to find the best result
                best_hit = None
                for hit in hits:
                    song_title = hit['result']['title'].lower()
                    song_artist = hit['result']['primary_artist']['name'].lower()
                    ...
                    # Skip if artist is clearly not the original
                    if any(skip_word in song_artist for skip_word in ['translation', 'srpski', 'genius', 'prevod']):
                        continue
