...
class Subgenres(Base):
    """Detailed subgenre catalog (e.g., nu metal, neo soul, trap)."""
    __tablename__ = 'subgenres'
    
    subgenre_id = Column(Integer, primary_key=True, autoincrement=True)
    subgenre_name = Column(String(100), nullable=False)
    parent_genre_id = Column(Integer, ForeignKey('genres.genre_id'), nullable=True)
    description = Column(Text, nullable=True)
    created_at = Column(DateTime, default=func.now())
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now())
    ...
    # Relationships
    parent_genre = relationship("Genres", foreign_keys=[parent_genre_id])
    song_subgenres = relationship("SongSubgenres", back_populates="subgenre")
    
    # Constraints
    __table_args__ = (
        UniqueConstraint('subgenre_name', 'parent_genre_id', name='uq_subgenre_parent'),
    )


class SongSubgenres(Base):
    """Many-to-many relationship between songs and subgenres with ranking."""
    __tablename__ = 'song_subgenres'
    
    song_subgenre_id = Column(Integer, primary_key=True, autoincrement=True)
    song_id = Column(Integer, ForeignKey('songs.song_id', ondelete='CASCADE'), nullable=False)
    subgenre_id = Column(Integer, ForeignKey('subgenres.subgenre_id', ondelete='CASCADE'), nullable=False)
    confidence_score = Column(Numeric(5, 4), nullable=True)  # More precision for subgenres
    source = Column(String(50), nullable=True)  # spotify, lastfm, rules, etc.
    rank = Column(Integer, default=1)  # 1 = primary subgenre, 2 = secondary, etc.
    created_at = Column(DateTime, default=func.now())
    
    # Relationships
    # song = relationship("Songs", back_populates="song_subgenres")
    subgenre = relationship("Subgenres", back_populates="song_subgenres")
    
    # Constraints
    __table_args__ = (
        UniqueConstraint('song_id', 'subgenre_id', name='uq_song_subgenre'),
    )
